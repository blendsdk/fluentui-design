 ...
 *
 * The committed skill lives at `.agents/skills/fluentui-design/` and is
 * regenerated by the skill generator. The published package must contain a
 * plain `skills/` directory instead, so this step copies the committed tree to
 * `skills/fluentui-design/`. It always replaces the destination, which keeps
 * the packaged copy an exact match of the committed one.
 *
...
export const SOURCE_DIR = path.join(".agents", "skills", "fluentui-design");

/** The packaged skill directory, relative to the project root. */
export const DEST_DIR = path.join("skills", "fluentui-design");
...
// Allow `node --import tsx scripts/assemble.ts` to run the CLI.

#!/usr/bin/env node
/**
 * Release tool for the `fluentui-design` package.
 *
 * The skill ships as one package, so "the skill version" and "the published
 * package version" are the same value. This tool derives the next version from
 * the conventional commit messages written since the last git tag, writes it to
 * `package.json` and `package-lock.json` in one step, records it in
 ...
 *
 * Every changelog section also records the pinned baseline: the Fluent UI
 * library version and the API-fact commit the skill was built against. A reader
 * can then tell which upstream the skill describes without opening the facts
 * file.
 *
 * Usage:
...
/** The pinned-baseline facts file, relative to the repository root. */
const FRESHNESS_FILE = join(ROOT, "facts", "freshness.json");
...
export function parseCommit(subject, body = "") {
  ...
  if (!match) return { type: "other", scope: null, breaking, subject: trimmed };

  const scope = match[2] || null;
  const text = scope ? `${scope}: ${match[4]}` : match[4];
  return { type: match[1].toLowerCase(), scope, breaking, subject: text };
}
 ...
 *
 * The section always opens with a Baseline table, so every changelog entry
 * records which Fluent UI library version and which API-fact commit the skill
 * was built against. Breaking changes are listed before the grouped changes.
 *
 ...
 */
export function buildChangelogEntry(version, date, commits, baseline) {
  if (!baseline || !baseline.packageVersion || !baseline.sourceCommit) {
    throw new Error("buildChangelogEntry requires a baseline with packageVersion and sourceCommit");
  }
  ...
  lines.push(`| \`@fluentui/react-components\` | ${baseline.packageVersion} |`);
  lines.push(`| \`fluentui-mcp\` facts commit | ${baseline.sourceCommit} |`);
  lines.push("");

  const breaking = parsed.filter((commit) => commit.breaking);
  if (breaking.length > 0) {
    lines.push("### Breaking Changes", "");
    for (const commit of breaking) lines.push(`- ${commit.subject}`);
    lines.push("");
  }
  ...
  for (const commit of parsed) {
    ...
    grouped.get(label).push(commit.subject);
  }
...
export function readBaseline() {
  const manifest = JSON.parse(readFileSync(FRESHNESS_FILE, "utf-8"));
  const pinned = manifest?.pinned;
  if (
    !pinned ||
    typeof pinned.packageVersion !== "string" ||
    typeof pinned.sourceCommit !== "string"
  ) {
    throw new Error("facts/freshness.json is missing pinned.packageVersion or pinned.sourceCommit");
  }
  return { packageVersion: pinned.packageVersion, sourceCommit: pinned.sourceCommit };
}
 ...
 * @param baseline - Pinned baseline to record in the section.
 */
function updateChangelog(version, commits, baseline) {
  const path = join(ROOT, "CHANGELOG.md");
  const existing = readFileSync(path, "utf-8");
  const date = new Date().toISOString().slice(0, 10);
  const entry = buildChangelogEntry(version, date, commits, baseline);
 ...
 *
 * The release flow keeps every git, file, and npm operation behind this object
 * so a caller can supply test doubles instead of touching a real repository.
 *
...
function printUsage() {
  console.log(`Release the single fluentui-design package.
 ...
 *
 * @param argv - Arguments after the executable.
 * @param overrides - Operations to replace, used by tests to avoid side effects.
...
export function main(argv, overrides = {}) {
  ...
  try {
    if (command === "version" || command === "release") {
      if (!options.dryRun) deps.assertCleanTree();
      if (!deps.changedSinceLastTag()) {
        deps.error("error: no changes since the last tag; nothing to release");
        return 1;
      }

      const commits = deps.readCommits();
      const type = options.type === "auto" ? determineBump(commits) : options.type;
      const current = deps.currentVersion();
      const next = semverBump(current, type);
      deps.assertTagAbsent(next);

      deps.log(`Version: ${current} -> ${next} (${type})`);
      if (options.dryRun) {
        deps.log(`[dry-run] would set ${next}, update CHANGELOG.md, commit and tag v${next}`);
      } else {
        const baseline = deps.readBaseline();
        deps.applyVersion(next);
        deps.updateChangelog(next, commits, baseline);
        deps.commitAndTag(next, options);
        deps.log(`Tagged v${next}`);
      }
    ...
    deps.error(`error: ${caught instanceof Error ? caught.message : String(caught)}`);
    return 1;
  }
}

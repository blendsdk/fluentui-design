...
export function resolveSourceDir(moduleUrl: string, override?: string): string {
  ...
  const packaged = path.join(moduleDir, "..", "..", "skills", SKILL_DIR_NAME);
  const repository = path.join(moduleDir, "..", "..", ".agents", "skills", SKILL_DIR_NAME);

  for (const candidate of [packaged, repository]) {
...
export function detectClients(options: {
  ...
  for (const client of CLIENTS) {
    ...
    const present =
      options.exists(globalDir) ||
      options.exists(path.dirname(globalDir)) ||
      options.exists(projectDir);
...
function cleanLeftovers(targetDir: string): void {
  const leftover = new RegExp(
    `^(${escapeForRegExp(TEMP_PREFIX)}|${escapeForRegExp(BACKUP_PREFIX)})\\d+$`,
  );
...
function printUsage(): void {
  console.log(`Install and update the Fluent UI design Agent Skill.
